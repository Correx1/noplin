        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 lg:gap-20 items-start">
          ...
          >
            <div className="flex flex-col gap-3">
              <span className="font-[var(--font-display)] font-semibold text-[11px] tracking-[0.14em] uppercase text-cyan">Who We Are</span>
              <h2 className="font-[var(--font-display)] font-bold text-[clamp(1.75rem,4vw,2.75rem)] text-[var(--text-primary)] leading-[1.15]">Built Differently.<br/>Built for Results.</h2>
            </div>
            <div className="flex flex-col gap-4">
              <p className="font-[var(--font-body)] text-[17px] text-[var(--text-secondary)] leading-[1.85]">
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-3">
              <span className="font-[var(--font-display)] font-semibold text-[11px] tracking-[0.14em] uppercase text-electric">Our Purpose</span>
              <h2 className="font-[var(--font-display)] font-bold text-[clamp(1.5rem,3vw,2.25rem)] text-[var(--text-primary)] leading-[1.15]">Mission & Vision</h2>
            </div>
            ...
            <div className="flex flex-col gap-6">
              {missionVision.map((item, i) => (
                <motion.div key={item.label} initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: EASE, delay: i * 0.15 }}
                  className="flex flex-col sm:flex-row gap-5 px-6 py-8 bg-[var(--bg-card)] rounded-[20px]" 
                  style={{ border: `1px solid ${item.glow}20`, boxShadow: `0 0 40px ${item.glow}08` }}>
                  <div className="shrink-0 flex items-start">
                    <IconBox Icon={item.Icon} glow={item.glow} />
                  </div>
                  <div className="flex flex-col gap-2">
                    <p className="font-[var(--font-display)] font-semibold text-[13px] uppercase tracking-[0.1em]" style={{ color: item.glow }}>{item.label}</p>
                    <p className="font-[var(--font-body)] text-[15px] text-[var(--text-secondary)] leading-[1.6]">{item.text}</p>
                  </div>
                </motion.div>
              ))}
            </div>
