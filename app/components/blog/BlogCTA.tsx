    ...
    <section className="py-20 bg-(--bg-page) relative overflow-hidden">
      <div className="max-w-5xl mx-auto px-6">
        <motion.div 
          ...
          className="relative overflow-hidden rounded-[24px] bg-[#0A0A1F] border border-electric/30 p-8 md:p-12 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-10"
        >
          {/* Subtle background effects for premium feel */}
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 right-0 w-1/2 h-full bg-linear-to-l from-electric/10 to-transparent blur-3xl rounded-r-[24px]" />
            <div className="absolute bottom-0 left-0 w-1/2 h-full bg-linear-to-r from-electric/10 to-transparent blur-3xl rounded-l-[24px]" />
          </div>
          ...
          <div className="relative z-10 w-full max-w-md md:max-w-[420px]">
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              <div className="relative flex flex-col sm:flex-row items-stretch sm:items-center bg-white/5 border border-white/10 rounded-xl p-1.5 focus-within:border-electric/50 focus-within:bg-white/10 transition-colors duration-300">
                <input 
                  type="email" 
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="name@company.com" 
                  className="flex-1 bg-transparent px-4 py-3 sm:py-2 text-sm text-white placeholder:text-white/40 font-(--font-body) outline-none" 
                  required
              ...
              <p className="text-xs text-center md:text-left font-(--font-body) pl-2 min-h-[16px]">
                {status === 'success' ? (
